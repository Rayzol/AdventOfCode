...
public class Challenge4 extends Challenge {
	...
	void partTwo() {
		...
		Guard maxGuard = new Guard();
		for(Guard guard : guards) {
			if(guard.getMostSleptMinuteValue(guard.findMostSleptMinute()) > maxGuard.getMostSleptMinuteValue(maxGuard.findMostSleptMinute())) {
				maxGuard = guard;
			}
		}
		
		System.out.println("the answer for strategy 2 is: " + maxGuard.getId() * maxGuard.findMostSleptMinute());
